...
4. Optionally pick a **Purpose category** from the dropdown below the purpose field: a short, fixed list (QA regression, support case replication, feature development, customer demo/POC, training, performance benchmark, other, or an admin-configured variant of that list). It defaults to Unclassified and never blocks submission; you can set or change it later. When your admin has AI purpose classification turned on, the form calls out to the AI orchestrator in the background once you have typed enough purpose text or picked a topology, and prefills the still-untouched dropdown with its top-guessed category alongside a "Suggested: \<category\> \<NN\>%" line and the top three categories as percentage chips. Picking a category yourself always wins: once you touch the dropdown, no later suggestion overwrites it. If the call fails, the line just reads "Suggestion unavailable" and nothing else about creating the reservation changes.
...
- **Details** (default): basic info (id, purpose, dates, status), plus the purpose category. The reservation owner or an admin can set or change it here from the same dropdown as at creation, in any status including terminal ones, so a completed reservation can still be classified after the fact. Anyone else sees it as a read-only tag. If the reservation carries an AI suggestion (from the creation-time preview, or from the background classification pass that runs again when the reservation ends), a muted "AI suggested: \<category\> \<NN\>%" line appears under the category row, with the model's rationale as a tooltip and a "(dismissed)" marker if an admin has already dismissed it on the Purpose Review page. This line is read-only here; accepting, overriding, or dismissing a suggestion is an admin action on that page, not something you do from this modal.
...
- **Wiring status.** After a commit, HERD applies your wiring to the hardware one connection at a time, layer by layer, and the reservation detail modal's **Wiring** tab shows the per-connection result grouped into three sections: **L1 cross-connects** (each switch port pair), **L2 VLAN memberships** (each switch port and its allocated VLAN), and **L3 route pins** (each L3 switch's pinned route count). Every row carries an `ACTIVE` / `RELEASED` / `FAILED` status, the attempt count, and (on a failure) the driver's error; a failed row whose intent was a release is additionally marked "release pending". A row that fails to apply does not roll back your saved wiring; the intent stays durable. Most failures are transient (a driver timeout or a switch login blip): HERD auto-retries them in the background with backoff, and while the reservation is `ACTIVE` you can also press **Retry failed** to reattempt every retryable failed row across all layers now (the toast summarizes every outcome: reconnected, released, superseded, still failing, not retryable, frozen, or already retrying, the last meaning HERD's own background retry was working on that row at the moment you pressed the button, so it was left alone). A connection marked not retryable cannot be reapplied as recorded (its cabling path changed under the reservation); its recovery is to re-save the fork wiring, which re-resolves the path against current inventory, then retry. The Wiring tab stays readable after the reservation ends as part of the as-built record, but the retry action is offered only while the reservation is `ACTIVE`.
- **Fork version history.** The editor's fork history panel lists every commit (newest first) with author and timestamp. Each row also offers **Preview** (a read-only render of that version on top of your live draft, with editing, wiring, and Commit locked until you exit), **Diff** (compare a version against another version or your current draft, with added and removed devices and connections listed and color-highlighted on the canvas), and, while the reservation is `ACTIVE`, **Restore** (copies that version's canvas onto your draft; nothing is wired until you run Commit, which is what appends the new version carrying the "restored" marker). See [TOPOLOGY_EDITOR.md](TOPOLOGY_EDITOR.md#fork-version-preview-diff-and-restore-issue-622) for the full walkthrough.
...
- Selecting a Layer 3 Switch device opens a **Routing** panel where you can add, edit, or import routing intent (destination, next hop, interface, virtual router) for that switch; a route-count badge shows on its canvas node, turning red if a save or reservation finds a problem with it. A route's interface is checked against the canvas wiring, not just the switch: a physical interface whose port nothing on the canvas wires is refused, while an interface the config marks logical (a loopback, an SVI, a dummy interface inside a virtual router) is exempt. A route's virtual router is checked against the switch's own config (the virtual routers it declares and the interfaces each one owns), and applying such a route also needs a driver that declares virtual router support; a switch whose driver does not is refused at provisioning time with nothing sent to the device. See [TOPOLOGY_EDITOR.md](TOPOLOGY_EDITOR.md#layer-3-routing-intent-adr-0014-issue-34) for the full reason list and when it is checked.
