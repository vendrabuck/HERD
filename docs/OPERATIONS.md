...
- Recover: bring NATS up. The per-service outbox relays reconnect and drain their unpublished rows immediately on the next committed write, or on the next tick, so reservation and health events are delivered once (not lost), just delayed. No manual catch-up event is needed for these streams.
