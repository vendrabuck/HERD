...
| `INTERNAL_API_TOKEN` | 64-char hex string | auth, reservations, inventory, execution, cabling, user-profile, notifications, secrets, ai-orchestrator (every service except acl and integration) | Shared secret for service-to-service calls that use the `X-Internal-Token` header. Must match across all services that speak to each other. ai-orchestrator uses it only to call execution's internal validate-package endpoint for recipe drafting. integration has no internal-token call site (JWT-only facade), so it neither receives nor reads this variable. |
