...
| `AI_ORCHESTRATOR_SERVICE_URL` | `http://ai-orchestrator:8000` | Base URL the expiration sweep's purpose-classify reconciler (issue #646 phase 2) calls for `POST /internal/classify-purpose`. Passed explicitly in `docker-compose.yml` even though the default already matches the compose service name, mirroring `INVENTORY_SERVICE_URL` and the other inter-service URLs above. |
| `PURPOSE_CLASSIFY_BATCH_SIZE` | `20` | Maximum reservations the sweep reconciler classifies per tick, oldest `purpose_classify_requested_at` first (issue #646 phase 2, ADR 0013 point 8). |
| `PURPOSE_CLASSIFY_MAX_ATTEMPTS` | `3` | Per-row cap on sweep attempts. A 5xx, a timeout, or a transport error increments a row's `purpose_classify_attempts`; a row at the cap is skipped until an admin backfill or a config change makes it worth retrying. A `403` from the orchestrator (the feature is off there) does NOT count as an attempt and ends the whole tick immediately. |
| `PURPOSE_CLASSIFY_TIMEOUT_SECONDS` | `30.0` | Per-call timeout for the sweep reconciler's `POST /internal/classify-purpose` call. |
