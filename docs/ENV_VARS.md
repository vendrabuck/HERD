...
| `PURPOSE_CLASSIFY_INTERVAL_SECONDS` | `60` | How often the purpose-classify reconciler's own loop wakes up (issue #702). Split out of `EXPIRATION_INTERVAL_SECONDS` because this is the only sweep reconciler bound by an LLM call rather than a DB or fast HTTP round trip: `PURPOSE_CLASSIFY_BATCH_SIZE` rows per tick, each up to `PURPOSE_CLASSIFY_TIMEOUT_SECONDS`, means a single tick can run for minutes, and coupling that to the same loop as activation/completion/the provisioning backstops/fork archiving/wiring heal/pending prune would delay all of them behind a slow or hung orchestrator. The dev/test `docker-compose.override.yml` does not need to pin this: nothing in the test suites drives this loop directly, they call the reconciler function itself. |
