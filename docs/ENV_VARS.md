...
| `INTERNAL_API_TOKEN` | 64-char hex string | auth, reservations, inventory, execution, cabling, user-profile, notifications, secrets, ai-orchestrator, acl (every service except integration) | Shared secret for service-to-service calls that use the `X-Internal-Token` header. Must match across all services that speak to each other. ai-orchestrator uses it only to call execution's internal validate-package endpoint for recipe drafting. acl reads it as of issue #704, for the internal-token `POST /internal/check` route the apply-job fire-time authority re-check calls; `docker-compose.yml` passes it to the acl service block for that route. integration has no internal-token call site (JWT-only facade), so it neither receives nor reads this variable. |
...
| `LDAP_SYNC_RUN_STALE_SECONDS` | `7200` | Seconds a `ldap_sync_runs` row may sit at status `running` before the stale-run reaper flips it to `failed` (issue #528). Covers a hard process death mid-run (OOM kill, container crash, power loss) that never reaches the run's finalize step, leaving the row stuck. Reaped at the START of every sync run, whether triggered by the interval loop's tick or by sync-now, and INSIDE the run-serialization lock, so a sync-now-only deployment is covered too and the reap can never fire while a concurrent run holds the slot. Clamped at the point of use (with a warning, never a boot failure) to the larger of 60 seconds and TWICE the effective `LDAP_SYNC_INTERVAL_SECONDS`: the default 7200 is exactly twice the default 3600 interval, and a threshold at or below the interval could otherwise fail a run that is merely slow rather than dead. The admin UI renders a `running` row stale after 30 minutes client-side (`STALE_RUNNING_THRESHOLD_MS` in `frontend/src/api/ldapSync.ts`); that shorter figure is a DISPLAY hint only, deliberately below this backend timer, and it never writes to the row. This reaper is the one that corrects the audit record itself. |
...
| `AI_GENERATE_MAX_REPAIRS` | `2` | Bounded auto-repair for topology generation: how many times `/api/ai/generate` re-prompts the model after a repairable proposal-validation failure (unknown template, over-count, duplicate role, dangling edge role, element_to_element edge, self-loop edge, duplicate device-to-device edge) before giving up. `0` disables repair, so the first repairable mistake fails the request immediately. Range 0-5. See [AI_GENERATE.md](AI_GENERATE.md). |
...
| `AI_RESOLVER_CANDIDATES_PER_TEMPLATE` | `8` | How many AVAILABLE devices per proposed template the topology generator's cabling-aware resolver considers before choosing one per role. The resolver fetches at least as many devices as that template has roles, so this only ever widens the search; the extra candidates are what let it pick a combination the cabling graph can connect instead of the first N listed. Raise it for a sparsely cabled lab, where the reachable combination may be several devices down the list; each extra candidate costs more pathfind pairs (worst case quadratic in this value, per edge). Accepted range 1 to 50; a value outside it fails startup. See `docs/AI_GENERATE.md`. |
| `AI_RESOLVER_MAX_SEARCH_STEPS` | `5000` | Hard ceiling on candidate trials in the resolver's backtracking search, so a pathological proposal cannot make one request run unboundedly. On exhaustion the request fails with the same structured 422 an unconnectable proposal gets, naming the edges the search could not satisfy, rather than returning a partial or flagged topology. Must be at least 1. |
| `AI_PURPOSE_CLASSIFICATION_ENABLED` | `false` | Expose the lab-purpose-classification endpoints (`POST /api/ai/classify-purpose/preview`, user JWT; `POST /api/ai/internal/classify-purpose`, X-Internal-Token; issue #646 phase 2, ADR 0013 points 8-11). Default off; enforcement is at the route boundary (403 with the structured detail `{"error": "purpose_classification_disabled", "message": "Purpose classification is disabled"}` since issue #706), not just absence from docs, mirroring `AI_RECIPE_AUTHORING_ENABLED`. `GET /api/ai/status` reports the flag (folded with the provider being configured) as `purpose_classification` for conditional UI. See `docs/AI_PURPOSE_CLASSIFICATION.md`. |
| `AI_PURPOSE_INCLUDE_TRANSCRIPTS` | `true` | Whether the end-of-reservation classifier pass includes the reservation assistant's own transcripts for that reservation as a signal. Privacy note (ADR 0013 point 11): those transcripts were already sent to the configured AI provider once when the user chatted; this resends them, to the same provider, alongside the reservation's structured metadata. A deployment that must not resend user text sets this `false`; the creation-pass preview endpoint never uses transcripts regardless (no reservation exists yet). |
| `AI_DOCS_MANUAL_ENABLED` | `true` | Whether the built-in `herd-manual` documentation source is available to the assistant's `search_docs` and `read_doc` tools (ADR 0015, issue #31). The pages are copied into the ai-orchestrator image from `docs/manual/` and bind-mounted from the checkout under `make up`. Set `false` to remove the source; with no source enabled at all, the two tools are not advertised and are refused at the dispatch boundary. |
| `AI_DOCS_CORPUS_DIRS` | (empty) | Comma-separated `name=/absolute/path` entries, each a directory of `.md`, `.txt`, or `.html` files mounted into the ai-orchestrator container and exposed as one documentation source. A malformed entry, a missing directory, or an unreadable one is logged once at startup and skipped, never fatal. Names are lowercase (`a-z0-9`, `_`, `-`); `web` is reserved. |
| `AI_DOCS_WEB_ENABLED` | `false` | Whether `read_doc(source="web", path=<url>)` may fetch an allowlisted documentation URL. Default off, and enforced at the dispatch boundary rather than only by omitting the capability from the tool description, so a model that emits the call by name still fetches nothing. Requires `AI_DOCS_WEB_ALLOWED_PREFIXES`; with the flag on and no prefixes, nothing is fetchable. |
| `AI_DOCS_WEB_ALLOWED_PREFIXES` | (empty) | Comma-separated `https://` URL prefixes the web source may fetch. Each is normalized (lowercase host, no userinfo, no query) and matched as a plain string prefix, so end every prefix with `/`: `https://docs.example.com/frr/` admits `.../frr/routing.html` but not `.../frr-evil/`. Every address the host resolves to must be public (loopback, private, link-local, multicast, unspecified, reserved, and IPv4-mapped forms are refused), at most 3 redirects are followed with each hop re-checked the same way, and the response must be `text/html`, `text/plain`, or `text/markdown`. |
| `AI_DOCS_WEB_MAX_BYTES` | `524288` | Hard cap on the bytes read from one web documentation response before the body is cut. The converted text is then windowed under the per-tool result cap like any other tool result. |
| `AI_DOCS_INDEX_TTL_SECONDS` | `600` | How long a corpus index is reused before it is rebuilt on the next lookup. An operator can update a mounted corpus and see the change within this window without restarting the service; lower it for a corpus that changes often, raise it for a large static one. |
...
| `APPLY_JOB_MAX_HORIZON_DAYS` | `30` | How many days into the future `scheduled_for` may be set at schedule time (issue #704); requests beyond the horizon are rejected with 422. Bounds how stale a job's schedule-time authorization can get before it fires, on top of the fire-time re-check below. |
...
| Variable | Default | Purpose |
|---|---|---|
| `EXPIRATION_INTERVAL_SECONDS` | `60` | How often the expiration loop wakes up to activate `PENDING` reservations, complete `ACTIVE` ones whose windows closed, and emit upcoming-expiry reminders. The same tick runs the reconcilers that ride the sweep: the stranded-provisioning backstops (`PROVISION_TIMEOUT_SECONDS` below), fork archiving for ended reservations, the wiring-heal staging check, the missing-fork backstop, and the pending device-prune retry. Since issue #702 the purpose-classify reconciler no longer rides this loop; see `PURPOSE_CLASSIFY_INTERVAL_SECONDS` below. The dev/test `docker-compose.override.yml` pins this to `5` so sweep-dependent integration tests fit inside their timeout. |
...
| `OUTBOX_RELAY_TICK_SECONDS` | `5.0` | Transactional outbox relay (issue #21) fallback poll cadence in seconds: since issue #682, a committed write wakes the relay immediately (see `OUTBOX_WAKE_ON_WRITE` below), so this tick is now the backstop for a missed or not-yet-reconnected notification, and for any non-Postgres dialect. A NATS outage backs this off exponentially and a healthy tick resets it. |
| `OUTBOX_BATCH_SIZE` | `100` | Maximum outbox rows the relay publishes per drain. Each row is claimed with `FOR UPDATE SKIP LOCKED` and published with a `Nats-Msg-Id` header for publisher-side dedup. A drain that comes back exactly this many rows loops immediately for the remainder instead of waiting out `OUTBOX_RELAY_TICK_SECONDS`, since Postgres collapses one commit's writes into a single NOTIFY regardless of how many rows it staged. |
| `OUTBOX_RETENTION_SECONDS` | `604800` | How long published outbox rows are retained before the relay prunes them; default 7 days. |
| `OUTBOX_WAKE_ON_WRITE` | `true` | Issue #682: on Postgres, `enqueue_event` issues `SELECT pg_notify(channel, '')` on the committing session, and the relay runs a supervised LISTEN task that drains immediately on that notification instead of waiting out `OUTBOX_RELAY_TICK_SECONDS`. `false` is the ops escape hatch back to tick-only (pre-#682) behavior; the tick keeps running as the fallback either way. No-op on SQLite (unit tests) or any non-Postgres dialect. |
...
| `PURPOSE_CATEGORIES` | `qa_regression,support_case_replication,feature_development,customer_demo_poc,training,performance_benchmark,other` | Lab purpose classification taxonomy (issue #646 phase 1, decisions recorded for ADR 0013): a comma-separated list that replaces the default whole. `POST /` and `PATCH /{id}/purpose-category` validate `purpose_category` against this list (422 with `Unknown purpose_category '<value>'; allowed: <list>` otherwise); `GET /purpose-categories` returns it verbatim, in order. It is a plain string list, not a Postgres enum and not a categories table, so a reservation keeps its `purpose_category` value even after that value is removed from this list. Not exposed in the config UI (`config_schema.py`), matching the other reservations-specific tuning knobs on this list (`EXPIRATION_INTERVAL_SECONDS` and its neighbors above). |
| `AI_ORCHESTRATOR_SERVICE_URL` | `http://ai-orchestrator:8000` | Base URL the purpose-classify reconciler (issue #646 phase 2) calls for `POST /internal/classify-purpose`. Passed explicitly in `docker-compose.yml` even though the default already matches the compose service name, mirroring `INVENTORY_SERVICE_URL` and the other inter-service URLs above. |
| `PURPOSE_CLASSIFY_INTERVAL_SECONDS` | `60` | How often the purpose-classify reconciler's own loop wakes up (issue #702). Split out of `EXPIRATION_INTERVAL_SECONDS` because this is the only sweep reconciler bound by an LLM call rather than a DB or fast HTTP round trip: `PURPOSE_CLASSIFY_BATCH_SIZE` rows per tick, each up to `PURPOSE_CLASSIFY_TIMEOUT_SECONDS`, means a single tick can run for minutes, and coupling that to the same loop as activation/completion/the provisioning backstops/fork archiving/wiring heal/pending prune would delay all of them behind a slow or hung orchestrator. The dev/test `docker-compose.override.yml` pins this to `5` like `EXPIRATION_INTERVAL_SECONDS`: the AI-gated integration test in `tests/integration/test_purpose_review_flow.py` waits under the suite's 30-second cap for the loop to classify a cancelled reservation, which the 60-second default cannot meet. |
| `PURPOSE_CLASSIFY_BATCH_SIZE` | `20` | Maximum reservations the reconciler classifies per tick, oldest `purpose_classify_requested_at` first (issue #646 phase 2, ADR 0013 point 8). |
| `PURPOSE_CLASSIFY_MAX_ATTEMPTS` | `3` | Per-row cap on reconciler attempts. A "failed" outcome (a non-transient non-200 status, or a 200 with an unparseable body) increments a row's `purpose_classify_attempts`. Issue #706: a "transient" outcome (429, 502, 503, or 504, or any other transport error) and a "feature_off" or "forbidden" 403/404 do NOT count as an attempt and end the whole tick immediately, since none of those are per-row problems (a sustained transient condition or a stale internal token must not burn a row's whole attempt cap). A "timeout" outcome (2026-09-05 amendment, `httpx.TimeoutException`) DOES count as an attempt and does NOT end the tick: a timeout costs the provider up to `PURPOSE_CLASSIFY_TIMEOUT_SECONDS` trying to answer for that one row specifically, so it is per-row evidence rather than provider-wide evidence, and treating it as tick-ending previously let one slow row stall every row behind it forever. A row at the cap is skipped until an admin backfill (`POST /admin/purpose/backfill`) or a config change makes it worth retrying. |
| `PURPOSE_CLASSIFY_TIMEOUT_SECONDS` | `30.0` | Per-call timeout for the reconciler's `POST /internal/classify-purpose` call. Exceeding it counts an attempt against the row being classified (see `PURPOSE_CLASSIFY_MAX_ATTEMPTS` above) and moves the reconciler on to the next row in the same tick, rather than ending the tick. |

The execution service runs the same outbox relay for the `device.health_transition` event and, since issue #682, honors the same four variables as reservations above (`OUTBOX_RELAY_TICK_SECONDS`, `OUTBOX_BATCH_SIZE`, `OUTBOX_RETENTION_SECONDS`, `OUTBOX_WAKE_ON_WRITE`), each defaulting exactly as `herd_common.outbox.run_outbox_relay` does (5s tick, 100 batch, 7-day retention, wake-on-write enabled).
