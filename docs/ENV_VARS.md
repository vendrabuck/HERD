...
| `PURPOSE_CLASSIFY_MAX_ATTEMPTS` | `3` | Per-row cap on reconciler attempts. Only a "failed" outcome increments a row's `purpose_classify_attempts`: a non-transient non-200 status, or a 200 with an unparseable body. Issue #706: a "transient" outcome (429, 502, 503, or 504, a timeout, or any other transport error) and a "feature_off" or "forbidden" 403/404 do NOT count as an attempt and end the whole tick immediately, since none of those are per-row problems (a sustained transient condition or a stale internal token must not burn a row's whole attempt cap). A row at the cap is skipped until an admin backfill or a config change makes it worth retrying. |
| `PURPOSE_CLASSIFY_TIMEOUT_SECONDS` | `30.0` | Per-call timeout for the reconciler's `POST /internal/classify-purpose` call. |
