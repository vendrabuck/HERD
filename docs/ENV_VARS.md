...
| `HEALTH_POLL_MAX_CONCURRENCY` | `1` | Maximum polls in flight at once within a replica (issue #24). Each poll runs a driver subprocess, so this bounds live subprocesses regardless of how many rows are due; polls past the limit wait and are reported as `polls_deferred` in the per-tick `health_tick` log. The default of 1 matches the pre-#24 strictly sequential firing. |
| `HEALTH_POLL_IN_USE_INTERVAL_SECONDS` | `0` | Poll interval for devices in the in-use tier, entered when a consumed `reservation.created`/`reservation.updated` event names the device and left on `completed`/`cancelled`/`failed` (issue #24). Moving to in-use also pulls the device's `next_poll_at` earlier (never later) so the faster cadence starts immediately. `0` (default) disables the override: the registry-resolved interval (device or template `poll_interval_seconds`) applies, exactly the pre-tier behavior. |
