...
| `LDAP_SYNC_RUN_STALE_SECONDS` | `3600` | Seconds a `ldap_sync_runs` row may sit at status `running` before the stale-run reaper flips it to `failed` (issue #528). Covers a hard process death mid-run (OOM kill, container crash, power loss) that never reaches the run's finalize step, leaving the row stuck. Reaped on the interval loop's retention cadence and, best-effort, at the start of every sync-now run, so a sync-now-only deployment is covered too. The admin UI already renders a `running` row stale after 30 minutes client-side; this is the longer, more conservative backend timer that eventually corrects the audit record itself. |
