...
| `get_device_config_schema` | in-process schema registry plus a template fetch | The config schema for the device's connection type: `{connection_type, schema, allowed_keys}` |
...
- The assistant is read-only by default. When `AI_WRITE_TOOLS_ENABLED=true`, it can also propose and schedule config changes routed through the existing apply pipeline; every write is a dry-run proposal that you must confirm in the UI before any real apply runs. With the flag off, only the read-only tools above are advertised and the assistant cannot write to inventory, cabling, reservations, or execution.
