...
- `GET /api/reservations/reports/utilization?start=&end=&status=` returns `UtilizationReport{total_hours, total_reservations, by_user, by_device, by_topology_type, by_day, by_group, execution_run_count, fleet, by_purpose, by_user_purpose, by_device_purpose, by_purpose_suggested}` (admin-only). `fleet` is `FleetSection{device_count, idle_device_count, window_hours, total_reserved_hours, utilization_pct, devices}` or `null` when inventory is unreachable. Each purpose bucket carries `purpose_category`, `reservations`, and `device_hours`; `by_user_purpose` and `by_device_purpose` add `user_id` and `device_id` respectively. `by_purpose_suggested` (issue #646 phase 2) shares `by_purpose`'s three-field shape but is a separate list, never merged into `by_purpose`.
- `GET /api/reservations/reports/utilization.csv?start=&end=&section=user|device|fleet|purpose|user_purpose|device_purpose|purpose_suggested&status=` returns a `text/csv` download with `Content-Disposition: attachment`. `section=template` is rejected with 422 because the template rollup depends on the inventory service and is computed client-side. `section=fleet` returns 503 `Inventory service is unreachable` when the inventory join cannot be made (the CSV has no way to degrade partially). The purpose sections are standalone (`purpose`, `user_purpose`, `device_purpose`, `purpose_suggested`) rather than an extra column on `user`/`device`, since a single user or device can span multiple categories in the window and a CSV row cannot carry more than one.
