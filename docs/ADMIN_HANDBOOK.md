...
- **About page** (Administration, then About): the version, build, and build date of the frontend and of all 12 services. Check it after an upgrade and before filing a bug. A row marked "differs" means that service is running a different version or build from the frontend, which usually means it was not rebuilt: `ai-orchestrator`, `notifications`, `acl`, and `user-profile` have no dev mount and need `docker compose up -d --build <service>` to pick up new code. A build of `dev` means the image was built outside the Makefile (or is dev-mounted) and is never flagged. Status has three states (issue #874): "reachable" (a `GET /version` that answered with a well-formed body), "unreachable" (the service did not answer at all), and "invalid response" (the call got a `200` back but the body did not match the expected shape, e.g. a proxy's HTML error page or a JSON body missing a required field), shown with dashed version/build/build-date cells and no skew badge, since a version comparison against a malformed body would be meaningless. `make version` on the host prints the build string a fresh build would carry. Version comparison tolerates the backend's PEP 440 spelling and the frontend's semver spelling of the same pre-release, including a matching release-candidate number (`0.6.0rc1` and `0.6.0-rc.1` compare equal; `0.6.0rc1` and `0.6.0-rc.2` do not, the rc number matters). Limits worth knowing: the build string identifies the commit an image was built FROM, not the bytes in it, so two images built from different uncommitted trees at the same commit carry the same `-dirty` string and compare as equal; and on a dev-mounted stack the string describes the image while the running code is the working tree. A matching row is strong evidence on a clean, tagged build and weak evidence on a dirty one.
