...
- **CSV export**: "Download CSV" button on each of the By User, By Device, and By Template tables. User/device CSVs come from the server; the template CSV is serialized in the browser from the same rollup the table uses.
...
- Per-user preferences live under `user_preferences.extras.notifications` in the user-profile service. Users manage them at `/settings`; admins don't need to touch prefs directly, but if a user reports silence the fastest check is their Settings page (channel toggle + per-event checkboxes; the `device.health_transition` toggle is fleet-wide).
- The consumer reads prefs via user-profile's internal endpoint, so `INTERNAL_API_TOKEN` must be identical on the notifications and user-profile containers (and on auth and reservations for the iter-2 recipient resolver). If you rotate the token, restart all four so none is stuck on the old value.
- NATS absence is non-fatal: the notifications REST API (list, mark-read, preferences) stays up even if JetStream is unreachable, and the consumers reconnect when NATS returns. Events published while NATS was down are lost unless the publisher retries.
- Iteration 1 of notifications notified only the reservation owner. Iteration 2 (ROADMAP #13) added health-transition fan-out to admins + active reservation holders, deduped, with emit-on-Nth-failure suppression. Email / Slack / webhook dispatchers are still future work; the channel framework is ready for them as peer dispatchers.
