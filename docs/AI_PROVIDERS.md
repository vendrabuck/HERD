...
- **`/api/ai/generate` returns "AI referenced unknown templates"**: the model proposed a template name that does not exist in your inventory. Either the stack is unseeded (`make seed` to populate; see [seedtools/](../seedtools/)) or your user account lacks visibility into the required device groups.
