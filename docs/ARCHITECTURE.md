...
- `ldap`: the auth service binds against a configured LDAP / Active Directory server on every login. Accounts are provisioned just-in-time (`auth_source='ldap'`, no local hash) on first successful bind; `/register` returns 409. HERD role assignment still happens inside HERD (roles never sync). Directory group sync (ADR 0011, `docs/design/0011-ldap-group-sync.md`) is in delivery with phases 1-5 of 6 landed: admin-managed group mappings, an on-demand fail-closed reconcile (`POST /api/auth/admin/ldap-sync/run`), an opt-in deactivation/reactivation sweep guarded by a circuit breaker, and a background interval loop (`LDAP_GROUP_SYNC_ENABLED`, `LDAP_SYNC_INTERVAL_SECONDS`) that runs the same reconcile on a schedule and prunes old `ldap_sync_runs` rows (`LDAP_SYNC_RUNS_RETENTION_DAYS`); only the admin UI (phase 6) remains.
