...
1. **Strict (raise 502):** user-facing reads that feed the UI. Current sites: inventory's `_fetch_user_group_ids` and `_fetch_user_group_names` raise `HTTPException(502)` on non-200/connection errors. Endpoints that want fail-open layer their own try/except over these.
...
- **Eleven Alembic migration trees, eleven Dockerfiles, eleven service configurations.** Cross-cutting changes (a new common field, a logging format change, a dependency bump) touch multiple services. Integration tests are the only end-to-end validation path for cross-service flows.
- **Operational surface.** A first-time deployer brings up a Postgres with 11 schemas, NATS with the `HERD_RESERVATIONS` and `HERD_HEALTH` streams plus three DLQ subjects, and 11 service containers behind Traefik. The `config` service mitigates this by handling first-start setup through the UI, but the surface itself is real.
