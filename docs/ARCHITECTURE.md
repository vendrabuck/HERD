...
- `ldap`: the auth service binds against a configured LDAP / Active Directory server on every login. Accounts are provisioned just-in-time (`auth_source='ldap'`, no local hash) on first successful bind; `/register` returns 409. HERD role assignment still happens inside HERD (roles never sync). Directory group sync (ADR 0011, `docs/design/0011-ldap-group-sync.md`) is in delivery with phases 1-4 of 6 landed: admin-managed group mappings, an on-demand fail-closed reconcile (`POST /api/auth/admin/ldap-sync/run`), and an opt-in deactivation/reactivation sweep guarded by a circuit breaker; the interval loop and admin UI arrive in phases 5-6.
