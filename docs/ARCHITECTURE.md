...
- **execution** (`execution-consumer`) triggers driver actions:
  ...
  - `reservation.updated`: no longer touches L1/L2/L3 wiring (ADR 0009 phase 7); an added device is conflict-checked and flipped to `RESERVED` but wires nothing until a fork save draws its connections, and a removed device's wiring is released by the reservations PATCH handler itself: it prunes the device's nodes and edges from the fork through cabling's save reconcile (fork_version bump, port-claim release) and stages the released delta, so the release rides the normal wiring_changed apply (the sweep's delta-less heal covers a missed staging). The event still drives dynamic instances behind removed devices tearing down and the issue #24 health-poll tier bookkeeping.
