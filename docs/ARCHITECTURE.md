...
- **execution** (`execution-consumer`) triggers driver actions:
  ...
  - `reservation.updated`: no longer touches L1/L2/L3 wiring (ADR 0009 phase 7); an added device is conflict-checked and flipped to `RESERVED` but wires nothing until a fork save draws its connections, and a removed device's wiring releases only once a fork save prunes its edges and stages a heal (the sweep's delta-less heal is the backstop). The event still drives dynamic instances behind removed devices tearing down and the issue #24 health-poll tier bookkeeping.
