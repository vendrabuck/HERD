...
- `ldap`: the auth service binds against a configured LDAP / Active Directory server on every login. Accounts are provisioned just-in-time (`auth_source='ldap'`, no local hash) on first successful bind; `/register` returns 409. HERD role assignment still happens inside HERD (roles never sync). Directory group sync (ADR 0011, `docs/design/0011-ldap-group-sync.md`) is fully delivered, all 6 phases: admin-managed group mappings, an on-demand fail-closed reconcile (`POST /api/auth/admin/ldap-sync/run`), an opt-in deactivation/reactivation sweep guarded by a circuit breaker, a background interval loop (`LDAP_GROUP_SYNC_ENABLED`, `LDAP_SYNC_INTERVAL_SECONDS`) that runs the same reconcile on a schedule and prunes old `ldap_sync_runs` rows (`LDAP_SYNC_RUNS_RETENTION_DAYS`), and the admin UI (`/admin/ldap-sync`) for mapping CRUD, sync-now, and run history.
...
2. **Fail-open (return None, callers treat as "don't restrict"):** visibility filters where blocking on an outage is worse than allowing through. Sites: `reservations._fetch_visible_device_ids` and `reservations.expiration` (which resolves exclusive device IDs via the best-effort `_fetch_devices_best_effort` and filters inline). `acl.auth_client.fetch_user_groups` is NOT in this set despite the similar shape: it returns `[]` on any non-200 or transport error, and every ACL check then evaluates against that empty group set, so an auth outage DENIES rather than allows (closed by default, the same posture as `herd_common.acl`).
...
Hardening: execution's consumer does not treat an event's claim about a reservation as authoritative on its own. Before running any handler for `reservation.created`, `reservation.updated`, `reservation.cancelled`, `reservation.completed`, `reservation.failed`, or `reservation.wiring_changed`, `process_reservation_message` corroborates the event against reservations' own record of the row (`GET /internal/{id}`, X-Internal-Token) via `_verify_reservation_event`: a terminal event needs any terminal status (COMPLETED, CANCELLED, or FAILED, not only the exact matching one, since a cancelled event for a row now reported COMPLETED still names a reservation that has genuinely ended); `reservation.created` needs PENDING_PROVISION or ACTIVE; `reservation.updated` needs any NON-terminal status (PENDING, PENDING_PROVISION, or ACTIVE), since the device-set PATCH stages it at function level, not only from its ACTIVE-only prune-marker branch, and `update_reservation` itself accepts either ACTIVE or PENDING; and `reservation.wiring_changed` needs ACTIVE specifically, since every `stage_wiring_changed` call site gates on it (cabling only has a fork to read once a reservation is ACTIVE). A mismatch, or a 404, acks the message without running the handler and logs one warning (`nats_event_unverified`) carrying the event, reservation_id, and reported status, never the full payload; a 5xx or transport error raises `TransientUpstreamError` so the message NAKs and retries instead of proceeding on an unanswerable check. `reservation.provision_requested` carries no entry in this table and enforces its own preconditions server-side. Without this check, anyone able to publish to the broker could drive a live reservation's teardown or provisioning by naming it in a forged message; see Deployment below for why the broker itself also needs to stay off any interface other than loopback.
...
**`HERD_HEALTH`** carries `herd.health.status_changed`. The execution service's health-poll scheduler publishes a `device.health_transition` event when a polled device's `consecutive_failures` crosses the configured threshold (bad_news) or resets to zero (recovery). Two services consume this stream, each with its own durable consumer: notifications (`notifications-health-consumer`) fans the event out as in-app notifications to all admins plus any users with an active reservation on the device, and, since issue #831 (PR #888), integration (`integration-webhooks-health-consumer`) fans it out to every registered outbound webhook subscribed to `device.health_transition`, the same delivery path its reservations consumer uses.

All consumers use `ConsumerConfig(max_deliver=5, ack_wait=30)` (issue #895: no `backoff` list, since JetStream silently used one to replace the server-side `ack_wait` with `backoff[0]`, measured 1s against nats-server 2.10.29, and it only ever timed ack-timeout redeliveries, never a NAK). A transient-error NAK instead passes an explicit `delay=` drawn from each service's `NATS_NAK_BACKOFF_SECONDS` schedule (`1,5,15,60,120` in production; see [ENV_VARS.md](ENV_VARS.md#nats)), and every consumer's durable is created OR UPDATED to match its `ConsumerConfig` (`herd_common.jetstream.ensure_consumer`) before subscribing, so a durable provisioned under the old config is upgraded rather than silently keeping its stale `backoff` forever on a persistent NATS volume (`make prod`). All consumers publish poison messages (JSON decode errors) and exhausted deliveries to service-scoped DLQ subjects: `herd.reservations.dlq.execution` (execution), `herd.reservations.dlq.notifications` (notifications, reservations stream), `herd.reservations.dlq.integration` (integration webhooks, reservations stream), `herd.health.dlq.notifications` (notifications, health stream), and `herd.health.dlq.integration` (integration webhooks, health stream). See [OPERATIONS.md](OPERATIONS.md#inspecting-the-nats-dlq) for inspection.
...
Since issue #682, the same `enqueue_event` call also issues `SELECT pg_notify()` on the committing session (on Postgres only), and the relay runs a supervised LISTEN task that wakes it the moment that notification arrives, so a write is drained in low single-digit milliseconds instead of waiting out the tick. The tick (`OUTBOX_RELAY_TICK_SECONDS`) stays the fallback cadence exactly as before for a missed or not-yet-reconnected notification, and outage backoff is unchanged; `OUTBOX_WAKE_ON_WRITE=false` disables the notify/listen path entirely and falls back to tick-only polling.
...
A third consumer on `HERD_RESERVATIONS` lives in the `integration` service (durable `integration-webhooks-consumer`, DLQ subject `herd.reservations.dlq.integration`): it fans each reservation event out to every registered outbound webhook (see below). Since issue #831 (PR #888), integration runs a second, independent durable consumer, `integration-webhooks-health-consumer`, on `HERD_HEALTH` (DLQ subject `herd.health.dlq.integration`), fanning `device.health_transition` events out to webhooks the same way.
...
**Outbound webhook consumer and delivery.** Admins register subscriptions (`POST /api/v1/webhooks`: target URL, subscribed event types, shared HMAC secret). The two durable consumers above (`integration-webhooks-consumer` on `HERD_RESERVATIONS`, `integration-webhooks-health-consumer` on `HERD_HEALTH`) both dispatch to the same handler, which loads every active subscription matching an event and POSTs a signed copy to each concurrently. Each delivery is signed `X-HERD-Signature: sha256=<hex>` (HMAC-SHA256 over the raw body bytes, keyed by the subscription secret; `herd_common.webhooks.sign_body`), retried with backoff, and recorded in a `webhook_deliveries` ledger that doubles as the at-least-once dedup record (unique on `(subscription_id, event_id)`) and the dead-letter record (`status="dead"` on retry exhaustion). A failing receiver lands a `dead` ledger row and never NAKs the NATS message, so one bad target cannot re-fan-out to the others or stall the stream. See [EXTERNAL_API.md](EXTERNAL_API.md).
...
`missing_device` reason an unknown id gets rather than leaking its existence.
...
- CI: four-job GitHub Action (backend, frontend, plus advisory integration and nos-dialect jobs), runs on push/PR to main.

## Deployment

Docker Compose for local dev and small prod deployments. Traefik is the only TLS-terminating component; backend services speak plain HTTP on the Docker network. PostgreSQL and NATS are single-node in the default compose; cluster them manually for HA. NATS carries no broker authentication, so its host port bindings (4222, 8222) and Postgres's (`POSTGRES_PORT`, default 5433) are loopback-only in `docker-compose.yml`, matching the Traefik dashboard's loopback bind (issue #708); reach either from a remote host through an SSH port-forward or `docker compose exec`, never by widening the compose binding.
