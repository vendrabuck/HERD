...
EMAIL = os.environ.get("SEED_EMAIL") or os.environ.get("SUPERADMIN_EMAIL", "lane.nelms1@gmail.com")
...
SRL_L2_DRIVER_DIR = os.path.join(_REPO_ROOT, "drivers", "srl_l2")
FRR_L3_DRIVER_DIR = os.path.join(_REPO_ROOT, "drivers", "frr_l3")
...
if __name__ == "__main__":
    ...
    # `--nos-only` stages just the NOS test lab (mirrors --acl-only): the
    # drivers, templates, devices, ports, and cabling from seed_nos_lab,
    # against an already seeded stack, without the full ~20 min default
    # population or anything else main() does.
    elif "--nos-only" in sys.argv:
        nos_client = httpx.Client(verify=False, timeout=30.0)
        nos_token = login(nos_client)
        nos_client.headers["Authorization"] = f"Bearer {nos_token}"
        seed_nos_lab(nos_client)
    else:
        main()
