...
- `Partial` Editable reservation topologies. Give each reservation an editable fork of its parent topology: edit loosely during the reservation, reconcile on save, and capture an immutable as-built record at teardown, so the master template stays clean while reservations evolve. Shipped so far: the per-reservation fork (fork, fork connections, and fork versions, created on activation), plus the cabling-side internal fork endpoints from ADR 0006 P3a (read, a loose canvas draft with no reconcile, save with set-arithmetic reconcile and cross-reservation port-claim 409s, and an idempotent as-built archive). Those endpoints are internal-only and nothing calls them yet, so the user-facing live-edit flow still mutates the shared parent topology. Wiring the reservations service to forward edits onto the fork, archiving the as-built record at teardown, and switching the frontend to the fork (ADR 0006 P3a phases 3 to 5) plus execution consuming fork wiring deltas (P3b) remain planned.
