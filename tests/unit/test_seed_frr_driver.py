...
from seedtools import client as seed_client
from seedtools import drivers, frr_demo

_REPO_ROOT = Path(__file__).resolve().parents[2]


def test_frr_driver_dir_points_at_the_real_package():
    # The builder must target the checked-in driver package, not a temp/inline copy.
    frr_dir = Path(drivers.FRR_DRIVER_DIR)
    assert frr_dir == _REPO_ROOT / "drivers" / "frr_mgmt"
    assert (frr_dir / "driver.py").is_file()
    assert (frr_dir / "driver_metadata.json").is_file()


def test_make_frr_driver_zip_packages_exactly_the_two_files():
    data = drivers._make_frr_driver_zip()
...
def test_zipped_driver_matches_source_on_disk():
    data = drivers._make_frr_driver_zip()
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
...
def test_zipped_metadata_advertises_dry_run():
    data = drivers._make_frr_driver_zip()
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
...
def test_make_frr_driver_zip_returns_none_when_package_missing(monkeypatch, tmp_path):
    # A checkout without the driver package must degrade gracefully (warn + skip),
    # not raise, so the rest of the seed still runs.
    monkeypatch.setattr(drivers, "FRR_DRIVER_DIR", str(tmp_path / "nonexistent"))
    assert drivers._make_frr_driver_zip() is None


def test_zipped_driver_publishes_commands_config_schema():
...
def test_get_or_create_driver_replaces_file_when_exists():
    ...
    did = drivers.get_or_create_driver(
        _FakeClient(),
    ...
    assert put_calls == [f"{seed_client.BASE}/inventory/drivers/{existing_id}/file"]


def test_get_or_create_driver_no_replace_by_default():
...
def test_seed_frr_topology_builds_two_node_chain():
    ...
    tid = frr_demo.seed_frr_topology(_FakeClient(), [r1, r2])
    assert tid == "topo-id"
...
def test_wait_for_active_returns_when_active(monkeypatch):
    # Poll loop: returns as soon as the reservation reports ACTIVE.
    monkeypatch.setattr(frr_demo.time, "sleep", lambda *_: None)
    ...
    assert frr_demo._wait_for_active(_FakeClient(), "res-1", timeout_s=10) == "res-1"


def test_wait_for_active_stops_on_terminal_state(monkeypatch):
    ...
    res = frr_demo._wait_for_active(_FakeClient(), "res-2", timeout_s=10)
    assert res == "res-2"
    assert calls["n"] == 1  # returned on the first terminal read, no spinning


def test_ensure_frr_reservation_reuses_bound_one(monkeypatch):
    ...
    res = frr_demo.ensure_frr_reservation(_FakeClient(), [r1], topology_id="t1")
    assert res == "res-existing"
    assert posted["n"] == 0  # reused, never created a second reservation


def test_ensure_frr_reservation_cancels_unbound_and_recreates_bound(monkeypatch):
    ...
    res = frr_demo.ensure_frr_reservation(_FakeClient(), [r1, r2], topology_id="t1")
