...
touched, on both HERD_RESERVATIONS and HERD_HEALTH. This is a read-only proof
against whatever consumers are already running against those streams; it
...
HERD_RESERVATIONS/HERD_HEALTH durables from a test.
"""
...
_DURABLES = [
    ("HERD_RESERVATIONS", "execution", "execution-consumer"),
    ("HERD_RESERVATIONS", "notifications", "notifications-consumer"),
    ("HERD_RESERVATIONS", "integration", "integration-webhooks-consumer"),
    ("HERD_HEALTH", "notifications", "notifications-health-consumer"),
    ("HERD_HEALTH", "integration", "integration-webhooks-health-consumer"),
]
...
async def test_consumers_have_real_ack_wait_and_no_backoff():
    ...
    try:
        js = nc.jetstream()
        failures = []
        for stream, service, durable in _DURABLES:
            try:
            ...
            if info.config.backoff:
                failures.append(
                    f"{service}'s durable {durable!r} on {stream}: "
                    f"backoff={info.config.backoff!r}, expected empty/None"
                )
        assert not failures, "\n".join(failures)
    finally:
        await nc.close()
