...
async def test_non_owner_canvas_edit_of_reserved_topology_is_blocked_409(
    admin_client, user_client, fresh_devices
):
    """The topology creator cannot rewire a topology that a DIFFERENT user holds a
    live reservation on; the cabling reservation lock returns 409.

    Topology RBAC (creator-or-admin) runs before the reservation lock, so the
    lock is only reachable by a non-admin who created the topology. The realistic
    scenario it guards: the intuser creates a shared topology, the admin reserves
    it, and the intuser then tries to change its wiring out from under that live
    reservation. Two ungrouped fresh devices avoid the cross-group cabling guard.
    """
    devices = await fresh_devices(2)
    a_id, b_id = devices[0]["id"], devices[1]["id"]

    connection_id = await _create_connection(admin_client, a_id, b_id)
    # The intuser creates and owns the topology (so it passes the creator RBAC
    # check on edit); the admin owns the live reservation on it.
    topology_id = await _create_topology(user_client)
    reservation_id: str | None = None
    try:
        first = await _set_canvas(user_client, topology_id, _canvas_with_edge(a_id, b_id))
        first.raise_for_status()
...
async def test_non_owner_name_only_edit_of_reserved_topology_is_allowed(
    ...
    """A name-only topology edit by the creator during someone else's live
    reservation is not gated: the lock only guards canvas (wiring) changes.

    Same actors as the 409 test (intuser owns the topology, admin owns the
    reservation), but the intuser PUTs a name change with no canvas_data, so
    canvas_changed is false, the lock does not apply, and the edit succeeds.
    """
    devices = await fresh_devices(2)
    a_id, b_id = devices[0]["id"], devices[1]["id"]
