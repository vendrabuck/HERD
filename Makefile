...
SERVICES := common auth inventory reservations cabling acl execution config ai-orchestrator user-profile notifications
DB_SERVICES := auth inventory reservations cabling acl execution user-profile notifications ai-orchestrator

# Coverage package name per service. Most are app/; common ships herd_common/.
cov_pkg = $(if $(filter common,$(1)),herd_common,app)

.PHONY: master master-quick master-clean clean-images everything \
...
# DB_SERVICES (config and ai-orchestrator have no DB, so they are excluded).
...
# poking around inside; config has no DB and ai-orchestrator stays stateless).
